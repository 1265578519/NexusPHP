...
--
-- 主机: localhost:3306
-- 生成日期: 2023 年 02 月 13 日 07:01
-- 服务器版本: 5.6.43-log
-- PHP 版本: 5.6.33

-- 找到数据库agent_allowed_family表单导入，由于php有缓存，24小时后生效
-- 该agent_allowed_family表只写入Transmission 4.0.0以上版本支持，无需清空表，直接原有表覆盖导入即可
-- 立即更新缓存命令：service memcached restart
-- 这个版本更新Transmission支持
...
(null, 'Transmission 4.0', 'Transmission 4.0.0', '/^-TR4([0-9])([0-9])([0-9])-/', 3, 'dec', '-TR4000-', '/^Transmission\\/4\\.([0-9])\\.([0-9])/', 2, 'dec', 'Transmission/4.0.0', 'no', 'yes', '', 0);
